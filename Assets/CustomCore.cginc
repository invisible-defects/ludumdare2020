...
half3 EmissionCustom(half3 rgb, half alpha) {
	if (alpha > 0.4 && alpha < 0.6) {
		return rgb * _EmissionColor.rgb;
	}
	else {
		return 0;
	}
...
half4 fragForwardBaseInternal(VertexOutputForwardBase i)
{
	...
	c.rgb += EmissionCustom(s.diffColor, s.rawAlpha);
...
#ifdef _EMISSION
	emissiveColor += EmissionCustom(s.diffColor, s.rawAlpha);//Emission(i.tex.xy);
#endif
